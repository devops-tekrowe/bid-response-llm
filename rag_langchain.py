...
def get_qdrant_client() -> QdrantClient:
    ...
    if not QDRANT_URL or not QDRANT_API_KEY:
        p_error("QDRANT_CLOUD_HOST or QDRANT_CLOUD_API_KEY missing from .env")
        raise ValueError("Set QDRANT_CLOUD_HOST and QDRANT_CLOUD_API_KEY in your .env file.")

    p_step(f"Connecting → {QDRANT_URL}")
    client = QdrantClient(url=QDRANT_URL, api_key=QDRANT_API_KEY)

    p_result("qdrant_url", QDRANT_URL)
...
def main():
    ...
    p_info("Mode: QUERY")
    p_info("Haven't ingested yet? Run:  python rag_langchain.py --ingest")
